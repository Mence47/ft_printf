...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/15 19:24:16 by rtroll            #+#    #+#             */
/*   Updated: 2019/01/15 19:24:21 by rtroll           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_prec_no_null(char *str, t_data data)
{
	...
	if (data.flags[3] == 1 && ft_check_null(str) && str[0])
	{
...
static char	*ft_no_width_prec(char *str, t_data data)
{
	...
	if (data.flags[3] == 1 && ft_check_null(str))
	{
...
static char	*ft_print_prec(char *prec, t_data data)
{
	if (data.width <= (int)ft_strlen(prec))
		return (ft_no_width_prec(prec, data));
